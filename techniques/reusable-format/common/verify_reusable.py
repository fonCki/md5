...
Common:
- MD5 equal, SHA-256 different, sizes equal
- first differing byte; identical-suffix boundary
- header hexdumps (first 96 bytes)

Format-specific:
- JPEG: find 2nd COM marker and report big-endian length + next-marker offsets
- PDF : locate %%EOF markers (first/last), trailing bytes after last EOF
- GZIP: parse header flags/fields; show FNAME; decompress payload(s) and hash

Outputs in OUT_DIR:
- analysis.md
- c1.head.hex / c2.head.hex
- (optional) per-format notes
"""
...
def jpeg_find_second_com(data: bytes) -> Optional[Tuple[int, int, int, int]]:
    """
    Return (marker_pos, len_pos, length, next_from_marker)
    for the 2nd COM segment (marker FF FE). Robust scan.
    """
...
def pdf_find_eofs(data: bytes) -> Tuple[int, int, int]:
    """
    Return (first_eof, last_eof, trailing_bytes_after_last_eof),
    where EOF token is '%%EOF' (5 bytes).
    """
    tok = b"%%EOF"
    first = data.find(tok)
    last = data.rfind(tok)
...
def resolve_outputs(out_dir: Path, fmt: str) -> Tuple[Path, Path, str]:
    """
    Prefer manifest.json (artifacts list). Otherwise, fall back to defaults.
    For gzip, also accept *.tar.gz and any *.gz via globbing.
    Returns (path1, path2, ext_string_for_printing).
    """
    ...
    if man.exists():
        try:
            ...
            if len(arts) >= 2:
                p1 = out_dir / arts[0]
                p2 = out_dir / arts[1]
                # ext string is just for headings; derive from suffix(es)
                # pick the longest composite suffix (e.g., .tar.gz)
    ...
    if fmt == "gzip":
        # look for the most specific *gz names (prefers .tar.gz over .gz)
    ...
    raise SystemExit(f"Unsupported format for output resolution: {fmt}")
...
def analyze(fmt: str, out_dir: Path, inputs_dir: Optional[Path], emit_tools: bool) -> None:
    ...
    # select outputs (now robust to .tar.gz via manifest/glob)
    ...
    fd = first_diff(c1, c2)
    if not fd:
        raise SystemExit("Files are identical (unexpected for this demo).")
    off0, b1, b2 = fd
    ...
    fmt_lines: List[str] = []
    if fmt == "jpeg":
        # list diffs inside 0..73 like before
        ...
        # decompress (all members) and hash
        try:
...
if __name__ == "__main__":
    ...
    p.add_argument("--inputs-dir")   # reserved (JPEG provenance if you add it later)
