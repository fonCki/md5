...
Analyzes binary collision pairs and final text pairs
...
def find_collision_block_bounds(a: bytes,b: bytes,first_diff_offset: int) -> Tuple[int,int]:
    # nota: fastcoll collision blocks are typically 128 bytes
    start=first_diff_offset
    # round down to nearest 64-byte boundary
    start=(start//64)*64
    end=start+128

    # extend end to include all diffs
    i=end
...
def analyze(out_dir: Path) -> None:
    ...
    # TODO revisar: verificar MD5 preservation
